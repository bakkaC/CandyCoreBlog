...
![由于在 TTI 之前有两个任务执行时间超过 50 毫秒，TBT 为 45 毫秒，这两个任务分别超过了 50 毫秒阈值 30 毫秒和 15 毫秒。总阻塞时间是这些值的累积：30 毫秒 + 15 毫秒 \\\\\\\= 45 毫秒。](assets/性能优化：包体积%20并发模式视角/imagesThe_TBT_is_45ms_since_we_have_two_tasks_that_took_longer_than_50ms_be.png)
...
![交互式下一次绘制为 250 毫秒，因为这是测量的最高视觉延迟](assets/性能优化：包体积%20并发模式视角/imagesThe_Interaction_to_Next_Paint_is_250ms_as_its_the_highest_measured_vi.png)
...
![](assets/性能优化：包体积%20并发模式视角/imagesimage.png)
...
![](assets/性能优化：包体积%20并发模式视角/imagesThe_main_thread_is_responsible_for_handling_tasks_one_by_one.png)
...
![在渲染低优先级组件（粉色）时，React 会让出主线程以检查更重要的任务](assets/性能优化：包体积%20并发模式视角/imagesWhen_rendering_the_low-priority_components_(pink)_React_yields_back_t.png)
...
![](assets/性能优化：包体积%20并发模式视角/imagesInstead_of_a_single_non-interruptible_task_for_every_render_the_concu.png)
...
![React 基于用户交互而暂停当前渲染，迫使它优先渲染另一个更新](assets/性能优化：包体积%20并发模式视角/imagesReact_pauses_the_current_render_based_on_a_user_interaction_that_force.png)
...
![](assets/性能优化：包体积%20并发模式视角/imagesimage-1.png)
...
![](assets/性能优化：包体积%20并发模式视角/imagesimage-2.png)
